...
for params in train_params:
    ...
    for epoch in epochs:
        for eps in rate:
            wb[sheet_name][f"{column_names[column_out_offset + 1]}{row_offset}"] = (
                f"loss = {loss_func_name}, "
                f"epochs = {epoch}, "
                f"rate = {eps}, "
            ...
            wb[sheet_name][
                f"{column_names[column_offset]}{row_offset + 1}"
            ] = "loss = MSE"

            wb[validate_name][f"{column_names[column_out_offset + 1]}{row_offset}"] = (
                f"loss = {loss_func_name}, "
                f"epochs = {epoch}, "
                f"rate = {eps}, "
                f"validation split = {validation_split}, "
            ...
            for func_name in names:
                ...
                for net_num, net_history in enumerate(history):
                    ...
                    for metric_num, train_metrics in enumerate(net_history.history):
                        wb[sheet_name][
                            f"{column_names[column_out_offset]}{row_offset + 1 + metric_num * row_per_metric}"
                        ] = f"metric = {train_metrics}"
                        wb[sheet_name][
                            f"{column_names[column_out_offset + column_offset + 1]}{row_offset + 1 + metric_num * row_per_metric}"
                        ] = f"{func_name}"
                        wb[sheet_name][
                            f"{column_names[column_out_offset]}{row_offset + net_num + 1 + metric_num * row_per_metric + 1}"
                        ] = f"{net_shape[2:]}"
                        wb[sheet_name][
                            f"{column_names[column_out_offset + column_offset + 1]}{row_offset + net_num + 1 + metric_num * row_per_metric + 1}"
                        ] = f"{net_history.history[train_metrics][-1]}"
                ...
                for net_num, net in enumerate(nets):
                    ...
                    for metric_num, val_metrics in enumerate(validation_metrics):
                        loss_val = validation_metrics[val_metrics](
                            tf.convert_to_tensor(y_val, dtype=tf.float32),
                            net.feedforward(x_val),
                        )
                        wb[validate_name][
                            f"{column_names[column_out_offset]}{row_validate_offset + 1 + metric_num * row_per_metric}"
                        ] = f"metric = {val_metrics}"
                        wb[validate_name][
                            f"{column_names[column_out_offset + column_offset + 1]}{row_validate_offset + 1 + metric_num * row_per_metric}"
                        ] = f"{func_name}"
                        wb[validate_name][
                            f"{column_names[column_out_offset]}{row_validate_offset + net_num + 1 + metric_num * row_per_metric + 1}"
                        ] = f"{net_shape}"
                        wb[validate_name][
                            f"{column_names[column_out_offset + column_offset + 1]}{row_validate_offset + net_num + 1 + metric_num * row_per_metric + 1}"
                        ] = f"{loss_val}"
    ...
    date_str = date.strftime("%I_%M_%S%p on %B %d_pr, %Y")
