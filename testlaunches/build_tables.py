...
def prepare_interval(interval: Tuple[float, float], step: float, distr="uniform"):
    ...
    if distr == "uniform":
        ...
        d = distributions[distr](loc=a + (b - a) / 2, scale=(b - a) / 2, size=2 * points_count)
    elif distr == "binom" or distr == "nbinom":
        d = distributions[distr](n=points_count * 30, p=0.5, loc=0, size=points_count * 2) / 30 / points_count * scale + a
...
list_sol_functions = [LF_ODE_1_solution, LF_ODE_2_solution, LF_ODE_3_solution, ST_LF_ODE_1_solution,
                      LH_ODE_1_solution, LH_ODE_2_solution,
                      S_ODE_1_solution]
...
if __name__ == "__main__":
    for d in distributions:
        ...
        for i in range(3):
            ...
            x = prepare_interval(interval_for_table_func[i], step, distr=d)
            table = func(x)
            np.savetxt(f"./solution_tables/{d}/{func.__name__}.csv", table, delimiter=",")

        step = 0.05
        for i in range(0, 4):
            func = list_sol_functions[i]
            x = prepare_interval((0, 1), step, distr=d)
            y = [func(i) for i in x]
            table = np.array([x, y])
        ...
        for i in range(4, 6):
            func = list_sol_functions[i]
            x = prepare_interval((0.1, 1), step, distr=d)
            y = [func(i) for i in x]
            table = np.array([x, y])
            table = table.transpose()
            np.savetxt(f"./solution_tables/{d}/{func.__name__}.csv", table, delimiter=",")
    ...
    for i in range(3):
        ...
        table = func(x)
        np.savetxt(f"./solution_tables/validation_data/{func.__name__}.csv", table, delimiter=",")
